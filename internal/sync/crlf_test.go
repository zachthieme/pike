...
// TestSync_BareCRNotesPushLinkAndNoOp drives a full Push over notes whose final,
// unterminated line ends in a bare "\r" — from a truncated write or classic
// CR-only endings. The scanner drops that trailing "\r", so the eligible Task's
// line matches by content; the first Sync links it with one Add and no Delete, no
// Warnings and no failures; the notes keep their bare-CR ending; a second Sync
// over the now-linked notes touches HEY only to list.
func TestSync_BareCRNotesPushLinkAndNoOp(t *testing.T) {
	now := time.Now()
	dir := t.TempDir()
	notes := filepath.Join(dir, "notes.md")
	original := "# Notes\r\n- [ ] Buy milk @today\r"
	...
	// The linked line kept its bare-CR ending, and so did the header's CRLF.
	want := "# Notes\r\n- [ ] Buy milk @today @hey(h_new1)\r"
	if got, _ := os.ReadFile(notes); string(got) != want {
		t.Errorf("notes after push\n got: %q\nwant: %q", string(got), want)
	}

	// Second Sync over the now-linked notes: nothing to push, HEY touched only to
	// list, and the file keeps its bare-CR ending.
	addsBefore := len(client.added)
	mutationsBefore := len(client.mutations)
	listsBefore := client.listed
