...
func (r *Report) WriteText(w io.Writer) error {
	if r.DryRun {
		_, err := fmt.Fprintf(w,
			...
			r.WouldPush, r.WouldImport, r.WouldComplete, r.WouldUncomplete, r.WouldRetitle, r.WouldRecreate, r.WouldUnlink, r.WouldOrphan, r.ExistingLinks)
		return err
	}
	_, err := fmt.Fprintf(w,
		"Sync\n  %d task(s) pushed to HEY\n  %d task(s) failed to push\n  %d todo(s) imported to the inbox\n  %d item(s) completed\n  %d item(s) uncompleted\n  %d task(s) retitled\n  %d todo(s) re-created\n  %d link(s) unlinked\n  %d orphan(s) reported\n  %d link(s) already exist\n",
		r.Pushed, r.Failed, r.Imported, r.Completed, r.Uncompleted, r.Retitled, r.Recreated, r.Unlinked, r.Orphans, r.ExistingLinks)
	return err
}
