...
- [user] A notes file whose final line ends in a bare `\r` (from a truncated write or classic CR-only line endings) is now handled correctly: the trailing `\r` is treated as that line's ending — matching how the scanner reads it — so Sync mutations apply instead of being refused forever, and appending a task line (a `--sync` import, or the TUI's new-task action) no longer appends a doubled CR that corrupts the previous last line (#28).
...
- [user] Appending a task line (a `--sync` import, or the TUI's new-task action) to a notes file whose final line ends in two or more carriage returns no longer silently drops a `\r` from that last line: the appended line is separated with a `\r\n` so the preceding line's scanner-visible content is left unchanged (#28).
